...
CONTINUE:
	...
	applyCommits := func(upto uint64) {
		...
		if n.paused {
			n.hcommit = upto
			n.debug("Paused, not applying %d", upto)
			return
		}
		for index := n.commit + 1; index <= upto; index++ {
			if err := n.applyCommit(index); err != nil {
				break
			}
		}
	...
	// Apply any previously stored entries first so that an older committed
	// peer-state update cannot overwrite membership changes from the current
	// append entry.
	preCommit := min(ae.commit, ae.pindex)
	applyCommits(preCommit)
	...
	// If this append arrived already committed, apply it only after processing its side effects.
	applyCommits(aeCommit)
